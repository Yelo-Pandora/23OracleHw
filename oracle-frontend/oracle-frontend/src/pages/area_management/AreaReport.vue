    ...
    <div v-else class="report-results">
      ...
      <div class="summary-cards">
        <div class="summary-card">
          ...
          <div class="card-value">{{ summary.totalareas }}</div>
        </div>
      ...
      <table class="report-table">
        ...
        <tbody>
          <tr v-for="item in reportData" :key="item.areaId">
            <td>{{ item.areaId }}</td>
...
const summary = computed(() => {
  ...
  return {
    totalareas: reportData.value.length,
...
const generateReport = async () => {
  ...
  try {
    ...
    const response = await axios.get('/api/area/report', { params });
  ...
  link.setAttribute('download', `区域统计报表_${filters.startDate}_至_${filters.endDate}.csv`);
...
const exportPDF = async () => {
  ...
  try {
    ...
    pdf.save(`区域统计报表_${filters.startDate}_至_${filters.endDate}.pdf`);
  } catch (err) {
