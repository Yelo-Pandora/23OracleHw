<template>
  <div class="temp-auth-window" v-if="show">
    <div class="temp-auth-content">
      <div class="temp-auth-title">
        {{ showAll ? '全部员工临时权限' : `员工${currentEmployeeId}临时权限` }}
      </div>
      <div class="temp-auth-header">
        <input class="temp-auth-search" v-model="localSearch" placeholder="搜索..." />
        <button class="temp-auth-toggle-btn" :disabled="!canToggleShowAll" @click="$emit('toggleShowAll')">
          {{ showAll ? `只显示员工${currentEmployeeId}的权限` : '显示全部' }}
        </button>
      </div>
      <div class="temp-auth-divider"></div>
      <div class="temp-auth-table-wrap">
        <table class="temp-auth-table">
          <thead>
            <tr>
              <th>员工账号</th>
              <th>员工昵称</th>
              <th>员工ID</th>
              <th>活动ID</th>
              <th>员工临时权限</th>
              <th>操作</th>
            </tr>
          </thead>
          <tbody class="temp-auth-table-body">
            <tr v-for="item in filteredList" :key="item.event_id">
              <td>{{ item.account }}</td>
              <td>{{ item.username }}</td>
              <td>{{ item.staffId }}</td>
              <td>{{ item.event_id }}</td>
              <td>{{ item.temp_auth }}</td>
              <td>
                <button class="action-btn temp_auth_edit" @click="$emit('edit', item)">编辑</button>
              </td>
            </tr>
          </tbody>
        </table>
      </div>
      <div>
        <button class="temp-auth-close" @click="$emit('close')">关闭</button>
      </div>
    </div>
  </div>
</template>

<script setup>
import { computed, ref, watch } from 'vue';

const props = defineProps({
  show: Boolean,
  showAll: Boolean,
  currentEmployeeId: [String, Number],
  tempAuthList: Array,
  search: String,
  canToggleShowAll: {
    type: Boolean,
    default: true
  }
});
const emit = defineEmits(['close', 'toggleShowAll', 'edit', 'update:search']);
const localSearch = ref(props.search || '');
watch(() => props.search, val => { localSearch.value = val; });
watch(localSearch, val => { emit('update:search', val); });

const filteredList = computed(() => {
  let list = props.showAll
    ? props.tempAuthList
    : props.tempAuthList.filter(item => item.staffId === props.currentEmployeeId);
  const keyword = (localSearch.value || '').trim().toLowerCase();
  if (keyword) {
    list = list.filter(item => {
      return (
        (item.account && item.account.toLowerCase().includes(keyword)) ||
        (item.username && item.username.toLowerCase().includes(keyword)) ||
        (item.staffId && String(item.staffId).includes(keyword)) ||
        (item.event_id && String(item.event_id).includes(keyword)) ||
        (item.temp_auth && String(item.temp_auth).toLowerCase().includes(keyword))
      );
    });
  }
  return list.slice().sort((a, b) => {
    const idA = Number(a.event_id);
    const idB = Number(b.event_id);
    if (isNaN(idA) || isNaN(idB)) return String(a.event_id).localeCompare(String(b.event_id));
    return idA - idB;
  });
});
</script>

<style scoped>
.action-btn {
    margin: 0 4px;
    padding: 6px 18px;
    border: none;
    border-radius: 6px;
    font-size: 14px;
    font-weight: 500;
    cursor: pointer;
    transition: background 0.2s, color 0.2s;
}

.temp-auth-toggle-btn {
  background: #f7b731;
  color: #fff;
  border: none;
  border-radius: 8px;
  font-size: 15px;
  font-weight: bold;
  padding: 8px 20px;
  margin-left: 12px;
  cursor: pointer;
  transition: background 0.2s;
  opacity: 1;
}
.temp-auth-toggle-btn:disabled {
  background: #e0e0e0;
  color: #aaa;
  cursor: not-allowed;
  opacity: 0.7;
}
.temp-auth-toggle-btn:not(:disabled):hover {
  background: #f7c676;
  color: #fff;
}
.temp_auth_edit {
  background: #4caf50;
  color: #fff;
}
.temp_auth_edit:hover {
  background: #6fdc7a;
}
.temp-auth-window {
  position: fixed;
  top: 0; left: 0;
  width: 100vw; height: 100vh;
  background: rgba(0,0,0,0.18);
  z-index: 9999;
  display: flex;
  align-items: center;
  justify-content: center;
}
.temp-auth-content {
  background: #fff;
  border-radius: 16px;
  border: 2px solid #e0e0e0;
  box-shadow: 0 8px 32px rgba(0,0,0,0.18);
  width: 50vw; height: 50vh;
  min-width: 380px; min-height: 180px;
  max-width: 900px; max-height: 700px;
  padding: 32px 36px;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: flex-start;
}
.temp-auth-title {
  width: 100%;
  text-align: center;
  font-size: 20px;
  font-weight: bold;
  margin-bottom: 10px;
}
.temp-auth-header {
  width: 100%;
  display: flex;
  justify-content: center;
  align-items: center;
  margin-bottom: 8px;
  gap: 12px;
}
.temp-auth-search {
  width: 70%;
  padding: 8px 12px;
  border-radius: 6px;
  border: 1px solid #ccc;
  font-size: 15px;
  outline: none;
  box-sizing: border-box;
}
.temp-auth-divider {
  width: 100%; height: 1.5px;
  background: #e0e0e0;
  margin: 8px 0 16px 0;
}
.temp-auth-table-wrap {
  flex: 1;
  margin: 0; padding: 0;
  max-height: 48vh; min-height: 120px;
  width: 100%;
  overflow-y: auto;
  position: relative;
}
.temp-auth-table {
  width: 100%;
  border-collapse: separate;
  border-spacing: 1;
  border-radius: 12px;
  overflow: hidden;
  table-layout: fixed;
}
.temp-auth-table th,
.temp-auth-table td {
  padding: 12px 16px;
  border-bottom: 1px solid #eee;
  text-align: center;
  word-break: break-all;
  white-space: pre-line;
  max-width: 140px;
  min-width: 100px;
  min-height: 40px;
}
.temp-auth-table th {
  background: #9cd1f6;
  font-weight: bold;
  position: sticky;
  top: 0;
  z-index: 2;
}
.temp-auth-table tr:hover { background: #cafefc; }
.temp-auth-table tr .c1 { background: #d8edf2; }
.temp-auth-table tr .c2 { background: #bdf0fc; }
.temp-auth-close {
  margin-top: 18px;
  padding: 8px 32px;
  border: none;
  border-radius: 8px;
  background: #499ffb;
  color: #fff;
  font-size: 16px;
  cursor: pointer;
  font-weight: bold;
  align-self: center;
}
.temp-auth-close:hover { background: #357ae8; }
.temp-auth-table-body {
  max-height: 36vh;
  min-height: 80px;
  width: 100%;
}
</style>