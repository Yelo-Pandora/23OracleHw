				...
				<table class="salary-detail-table">
					...
					<tbody>
						<tr v-for="emp in filteredList" :key="emp.staffId">
							...
							<td>{{ emp.bonus }}</td>
							<td>{{ emp.fine }}</td>
							<td>{{ calcTotalSalary(emp.salary, emp.bonus, emp.fine) }}</td>
...
const props = defineProps({
	...
	year: String,
	month: String,
	salaryList: Array // [{name, staffId, account, username, salary, bonus, fine, dept}]
});
...
const filteredList = computed(() => {
	...
	return (props.salaryList || []).filter(emp => {
		// 支持姓名、账号、昵称、部门模糊搜索
