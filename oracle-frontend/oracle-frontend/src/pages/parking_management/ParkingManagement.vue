...
const loadParkingData = () => {
  ...
  const currentLot = selectedParkingLot.value
  
  // 左侧区域：4行6列 = 24个车位
  for (let row = 1; row <= 4; row++) {
    for (let col = 1; col <= 6; col++) {
      const spaceId = parseInt(currentLot) * 1000 + (row - 1) * 6 + col
  ...
  for (let row = 1; row <= 4; row++) {
    for (let col = 1; col <= 6; col++) {
      const spaceId = parseInt(currentLot) * 1000 + 24 + (row - 1) * 6 + col
      const isOccupied = Math.random() > 0.6
      const isMaintenance = Math.random() > 0.9
...
const getSpaceByPosition = (area, row, col) => {
  return parkingSpaces.value.find(space => 
    space.area === area && space.row === row && space.col === col
