...
BEGIN
    ...
    -- 2. 清理测试车辆信息 (CAR表)
    SELECT COUNT(*) INTO v_car_count FROM CAR WHERE LICENSE_PLATE_NUMBER LIKE 'TEST%';
    DELETE FROM CAR WHERE LICENSE_PLATE_NUMBER LIKE 'TEST%';
...
SELECT 
    ...
    COUNT(*),
    CASE WHEN COUNT(*) = 0 THEN '✅ 清理成功' ELSE '❌ 仍有残留' END
FROM CAR WHERE LICENSE_PLATE_NUMBER LIKE 'TEST%'
UNION ALL
SELECT 
    '停车记录',
    COUNT(*),
...
FROM (
    ...
    WHERE PARKING_SPACE_ID BETWEEN 1001 AND 1020 
       ...
       OR PARKING_SPACE_ID BETWEEN 3001 AND 3025
    UNION ALL
    SELECT COUNT(*) FROM CAR WHERE LICENSE_PLATE_NUMBER LIKE 'TEST%'
    UNION ALL
